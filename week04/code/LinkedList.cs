...
public class LinkedList : IEnumerable<int>
{
    ...
    {
        // TODO Problem 3
        // Search for the node that matches 'value' starting at head.
        Node? curr = _head;
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                // If removing the head, reuse RemoveHead
                if (curr == _head)
                {
                    RemoveHead();
                }
                // If removing the tail, reuse RemoveTail
                else if (curr == _tail)
                {
                    RemoveTail();
                }
                ...
                {
                    curr.Prev!.Next = curr.Next; // Skip over current node
                    curr.Next!.Prev = curr.Prev; // Link next node back to previous
                }
